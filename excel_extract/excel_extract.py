...
if __name__ == '__main__':
    ...
    output_excel_dir = '../data/excel_output'
    ...
    for root,dirs,files in os.walk(input_Path):
        if len(files)>0:
            for file in files:
                ...
                cutImg_name=split_file[0]
                root_split = root.split("\\")
                sub_dir = root_split[-1]
                output_excel_subdir = output_excel_dir +"/"+ sub_dir
                if not os.path.exists(output_excel_subdir):
                    os.mkdir(output_excel_subdir)
                img_input_path = os.path.join(root,file)
                src_img, Net_img, contours = FindContours(img_input_path)
                get_Affine_Location(src_img, Net_img, contours,cutImg_name,output_excel_subdir)
